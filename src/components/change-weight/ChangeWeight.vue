                    ...
                    <a-timeline>
                        <a-timeline-item v-for="(item, index) in dataList" :key="index">
                            <h4 class="align-center">
                                <span class="opacity-70"> {{ item.oldWeight >= 0 ? item.oldWeight.toFixed(3) : "-" }}kg </span>
                            ...
                            <a-tag color="success" style="margin-bottom: 10px" v-if="item.paymentStatus == 'PAID'">{{ $t("l_Paid") }} ({{ $timeFormat(item.paymentdate, 1) || "-" }}) {{ item.price ? item.price.toFixed(2) : 0 }}$ </a-tag>
                            <h5>
                                {{ $t("l_Operator") }}:
                    ...
                    <a-form layout="vertical" :model="info" style="margin-bottom: 20px" @finish="onSubmit()">
                        <a-row :gutter="[8, 0]">
                            <a-col :xs="24" :sm="24" :lg="24">
                                <a-form-item :label="$t('l_Weight') + ' (kg)'" name="weight" :rules="[{ required: true }]">
                                    ...
                                    <a-upload-dragger
                                        ...
                                        <div class="upload-file-content">
                                            <p class="ant-upload-drag-icon">
                                                <span style="color: #1677ff; font-size: 35px" class="active material-symbols-outlined"> photo_camera </span>
                                            </p>
                            ...
                            <a-col :xs="24" :sm="24" :lg="24">
                                <a-form-item :label="$t('l_Description')" name="description" :rules="[{ required: false }]">
                                    <a-textarea class="w-100" style="width: 100%" v-model:value="info.description"></a-textarea>
                                </a-form-item>
                            </a-col>
                        </a-row>
...
export default {
    ...
    methods: {
        ...
        async customRequest(file, key, toSubmit) {
            ...
            http({
                url: "file/upload?folder=weight-update",
                data: Form,
            }).then((res) => {
                if (res.status == 0) {
