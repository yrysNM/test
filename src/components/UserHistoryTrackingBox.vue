<template>
    <a-modal destroyOnClose wrap-class-name="modal-content" width="90%" style="max-width: 580px" :maskClosable="true" v-model:visible="modal.visible" :title="$t('l_Order_JPID_history')" :centered="false" :footer="null" :confirm-loading="modal.confirmLoading" :afterClose="afterCloseModal">
            ...
            <a-timeline>
                <a-timeline-item v-for="(item, index) in dataInfoList" :key="index">
                    ...
                    <div class="flex flex-col gap-[5px]">
                        ...
                        <span v-if="!item.oldJpid && item.city && item.province"> {{ item.province }}, {{ item.city }} </span>
...
export default {
    ...
    methods: {
        ...
        async loadData() {
            ...
            http({
                url: config.nakedBaseURL + "service/protected/order/jpid-history/page",
                data: info,
            })
                .then((res) => {
