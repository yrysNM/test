<template>
    <a-modal destroyOnClose wrap-class-name="modal-content" width="90%" style="max-width: 580px" :maskClosable="true" v-model:visible="modal.visible" :title="$t('l_Order_branch_history')" :centered="false" :footer="null" :confirm-loading="modal.confirmLoading" :afterClose="afterCloseModal">
        <div class="mt-8 px-6 box-border rounded-md" v-if="dataInfoList.length">
            <a-timeline>
                <a-timeline-item v-for="(item, index) in dataInfoList" :key="index">
                    <template #dot>
                        <check-circle-outlined style="font-size: 23px" v-if="index === 0"></check-circle-outlined>
                        <div class="border border-black text-black p-2 box-border flex items-center justify-center w-[23px] h-[23px] rounded-full" v-else>
                    ...
                    <div class="flex flex-col gap-[5px]">
                                    ...
                                    <tr>
                                        <td>
                                            <div>
                                                ...
                                                <a-tag class="ml-1 border px-1 rounded" size="sm" v-if="item.oldStationType?.type === 'POINT'" color="warning">{{ item.oldStationType.type }}: {{ item.oldStationType.pointStationCode }}</a-tag>
                                            </div>
                                        </td>
                                        ...
                                        <td>
                                            <div>
                                                ...
                                                <a-tag v-if="item.newStationType?.type === 'POINT'" class="ml-1 border px-1 rounded" size="sm" color="warning"> {{ item.newStationType.type }}: {{ item.newStationType.pointStationCode }}</a-tag>
                                            </div>
                                        </td>
                                    </tr>
                                </tbody>
                        ...
                        <span v-if="!item.oldStationCode && item.city && item.province"> {{ item.province }}, {{ item.city }} </span>
...
export default {
    ...
    methods: {
        ...
        async loadData() {
            ...
            http({
                url: "order/station-history-page",
                data: info,
            })
                .then((res) => {
